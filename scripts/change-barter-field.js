...
const updateListings = async () => {
    ...
    listings.forEach(function updateListing(l, i) {
        const userId = l.id;
        const allowsBarter = l.attributes.publicData.allowsBarter &&
            l.attributes.publicData.allowsBarter[0] === "hasBarter";
        const emptyAllowsBarter = l.attributes.publicData.allowsBarter &&
            l.attributes.publicData.allowsBarter.length === 0;
        if (allowsBarter || emptyAllowsBarter) {
            console.log("Updating: " + l.attributes.title);
            integrationSdk.listings.update(
                {
                    id: userId,
                    publicData: {
                        allowsBarter: allowsBarter ? true : false
                    },
                },
                {
                    expand: true,
